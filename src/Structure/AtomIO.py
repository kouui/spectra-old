...
def read_general_info(_rs, _lns):
    ...
    for _i, _ln in enumerate(_lns[_rs:]):
        ...
        if _ln.split()[0].strip() == "CEparameter":
            _paramCE = int( _ln.split()[1].strip() ), int( _ln.split()[2].strip() )

    _re = _rs + _i + 1

    return _re, _Z, _Element, _nLevel, _nLine, _paramCE
...
def read_CE_info(_rs, _lns, _idxI, _idxJ, _Te, _table, _CE_type, _f1, _f2):
    ...
    for _i, _ln in enumerate(_lns[_rs:]):
        ...
        if _words[0] == "Temperature":
            _Te[:] = [float(_v.strip()) for _v in _words[1:]]
            continue
